...
if __name__ == "__main__":
    ...
    RESOURCES_PATH = Path('/home/keikei/project_landing/simulation_coast/rosbag/')

    rosbag = 'coast_land_3/coast_land_3_0.db3'
    ...
    storage_filter = rosbag2_py.StorageFilter(topics=['/drone0/self_localization/pose', '/drone0/motion_reference/trajectory', '/tf'])
    ...
    while reader.has_next():
        ...
        if topic == '/drone0/self_localization/pose':
            ...
            trajectory_x.append(msg.setpoints[0].position.x+1)
            trajectory_y.append(msg.setpoints[0].position.y)
            trajectory_z.append(msg.setpoints[0].position.z +5)
        elif topic == '/tf':
            for transform in msg.transforms:
                if transform.child_frame_id == 'boat_beacon':
                    pose_x_boat.append(transform.transform.translation.x)
                    pose_y_boat.append(transform.transform.translation.y)
                    pose_z_boat.append(transform.transform.translation.z + 1.0)

    # trim trajectory
    # pose_x = pose_x[3400:7200]
    # pose_x_boat = pose_x_boat[600:3872]
    # pose_y = pose_y[3400:7200]
    # pose_y_boat = pose_y_boat[600:3872]
    # pose_z = pose_z[3400:7200]
    # pose_z_boat = pose_z_boat[600:3872]

    title = "Trayectorias 3D en el aterrizaje en entorno marítimo"
    xlabel = "Eje X (m)"
    ylabel = "Eje Y (m)"
    zlabel = "Eje Z (m)"
    legend1 = "Trayectoria seguida por el dron"
    legend2 = "Trayectoria seguida por el barco"
    legend3 = "Trayectoria de referencia calculada"

    plot_3d(pose_x, pose_y, pose_z, pose_x_boat, pose_y_boat, pose_z_boat, trajectory_x, trajectory_y, trajectory_z, title, xlabel, ylabel, zlabel, legend1, legend2, legend3)

    pose_x_boat = pose_x_boat[2:]
    pose_y_boat = pose_y_boat[2:]
    pose_z_boat = pose_z_boat[2:]
    t = np.linspace(0, 1, len(pose_x))

    dist_xy = np.sqrt((np.array(pose_x) - np.array(pose_x_boat)) ** 2, (np.array(pose_y) - np.array(pose_y_boat)) ** 2)
    dist_z = np.array(pose_z) - np.array(pose_z_boat)

    title_dist = "Distancias entre el dron y el barco"
