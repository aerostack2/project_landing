...
if __name__ == "__main__":
    ...
    RESOURCES_PATH = Path('/home/keikei/project_landing/simulation_model/rosbag/')

    rosbag = 'land_sim_0/land_sim_0_0.db3'
    ...
    storage_filter = rosbag2_py.StorageFilter(topics=['/drone0/self_localization/pose', '/drone0/motion_reference/trajectory', '/rigid_bodies'])
    ...
    while reader.has_next():
        ...
        if topic == '/drone0/self_localization/pose':
            ...
            trajectory_x.append(msg.setpoints[0].position.x-1.0)
            trajectory_y.append(msg.setpoints[0].position.y+2.0)
            trajectory_z.append(msg.setpoints[0].position.z)
        elif topic == '/rigid_bodies':
            pose_x_plane.append(msg.rigidbodies[0].pose.position.x)
            pose_y_plane.append(msg.rigidbodies[0].pose.position.y)
            pose_z_plane.append(msg.rigidbodies[0].pose.position.z)

    # trim trajectory
    # pose_x = pose_x[3400:7200]
    # pose_x_plane = pose_x_plane[600:3872]
    # pose_y = pose_y[3400:7200]
    # pose_y_plane = pose_y_plane[600:3872]
    # pose_z = pose_z[3400:7200]
    # pose_z_plane = pose_z_plane[600:3872]

    title = "Trayectoria de aterrizaje en 3D"
    xlabel = "Eje X (m)"
    ylabel = "Eje Y (m)"
    zlabel = "Eje Z (m)"
    legend1 = "Trayectoria seguida por el dron"
    legend2 = "Trayectoria seguida por la plataforma"
    legend3 = "Trayectoria de referencia calculada"

    title_dist = "Distancias entre el dron y la plataforma móvil"
    ...
    plot_3d(pose_x, pose_y, pose_z, pose_x_plane, pose_y_plane, pose_z_plane, trajectory_x, trajectory_y, trajectory_z, title, xlabel, ylabel, zlabel, legend1, legend2, legend3)

    pose_x_plane = pose_x_plane[158:]
    pose_y_plane = pose_y_plane[158:]
    pose_z_plane = pose_z_plane[158:]
    t = np.linspace(0, 1, len(pose_x))

    dist_xy = np.sqrt((np.array(pose_x) - np.array(pose_x_plane)) ** 2, (np.array(pose_y) - np.array(pose_y_plane)) ** 2)
    dist_z = np.array(pose_z) - np.array(pose_z_plane)
